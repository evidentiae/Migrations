        ...
        For Each file In OpenFileDialog1.FileNames
            ...
            For i = 1 To book.Sheets.Count
                ...
                For y = 1 To (sheet.Range("A1").SpecialCells(Excel.XlCellType.xlCellTypeLastCell).Row)
                    Dim celll = getvalue(sheet, y, COLUMN)
                    ' neglecting empty boxes
                    For ui = 8 To 2 Step -1
                        If IsDate(getvalue(sheet, y, ui).Replace("()", "")) Then
                            Dim tmp1 = celll.Split
                            idd = tmp1(tmp1.Count - 1)

                            name = celll.Replace(idd, "").Trim

                            Exit For
                        End If
                    Next

                    If celll.Contains("Appointment Date:") Then
                        dat = celll.Replace("Appointment Date:", "")
                        notee = getvalue(sheet, y + 1, COLUMN)
                        xlWorkSheet.Cells(rd, 1) = idd
                        xlWorkSheet.Cells(rd, 2) = name
                        xlWorkSheet.Cells(rd, 3) = dat
                        xlWorkSheet.Cells(rd, 4) = notee
                        rd = rd + 1
                    End If
            ...
            xlWorkBook.SaveAs("C:\Users\developer\Desktop\bew\tmp\bew14.xlsx", Excel.XlFileFormat.xlWorkbookNormal, misValue, misValue, misValue, misValue,
