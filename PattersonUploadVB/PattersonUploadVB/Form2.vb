...
Public Class Form2
    ...
    Dim UploadPreviouslyUploaded = False ' no need this feature needed in dicom only
    ...
    Private Sub bw_DoWork()
        ...
        Try
            ...
            Label10.Invoke(Sub()
                               Label10.Text = "Removing Duplicates From Directory"
                           End Sub)

            c1.removeDupFromDirectory(files, Label2.Text)
            ...
            For i = 0 To filesCount - 1
                ...
                If Not c1.IsUploaded(Pat) Or UploadPreviouslyUploaded Then
                    ...
                    'MsgBox(Pat)
                    c1.log("code ; 444121 loading image Pat : " & Pat)
                    Dim ext = Path.GetExtension(Pat)

                    If ext.ToLower = ".jpeg" Or ext.ToLower = ".jpg" Then



                        Dim s(3) As String

                        s = c1.getPatientInfoFromJPEG(Pat)

                        If Not File.Exists(My.Computer.FileSystem.SpecialDirectories.Desktop & "\names.csv") Then
                            MsgBox("please add the name file")
                            Exit Sub
                        End If
                        Dim arrOfNames = c1.ReadIDFile(My.Computer.FileSystem.SpecialDirectories.Desktop & "\names.csv")



                        s = c1.GetPatientName(s(0), arrOfNames)
                        If s(0) = "" Or s(1) = "" Then
                            PatientNotInFile = PatientNotInFile + 1
                            c1.log("code ; 444122 no name found")
                        Else
                            Dim inactive As Boolean = CheckBox2.Checked

                            Dim id = c1.getPatientID(realm, logincookie, s(0), s(1), inactive, Country)
                            'MsgBox(uuid)
                            If (id Is Nothing Or id = "") And inactive Then
                                id = c1.getPatientID(realm, logincookie, s(0), s(1), Not inactive, Country)
                                'MsgBox("kk" & uuid)
                            End If
                            ...
                            If id IsNot Nothing And id <> "" Then
                                ...
                                If c1.uploadImageJpeg(Pat, realm, logincookie, id, Pat.Substring(Pat.LastIndexOf("\") + 1, Pat.Length - Pat.LastIndexOf("\") - 1), DateTime.Now.ToString("ddMMyyyyHHmmssfff")) Then
                                ...
                                End If
                            Else
                                c1.log("code : 444825 patient was not found in both active and inactive")
                                If CheckBox3.Checked Then
                                    My.Computer.FileSystem.CopyFile(Pat, My.Computer.FileSystem.SpecialDirectories.MyDocuments & "/PNF JPEG" & DNF & "/" & Regex.Replace(s(0), "[^A-Z a-z0-9\-/]", "") & "/" & DateTime.Now.ToString("dd-MM-yyyy HH-mm-ss-fff"))
                                    My.Computer.FileSystem.WriteAllText(My.Computer.FileSystem.SpecialDirectories.MyDocuments & "/PNF JPEG " & DNF & "/" & Regex.Replace(s(0), "[^A-Z a-z0-9\-/]", "") & "/info.txt", s(0) & "---" & s(1), False)
                                End If
                    ...
                    Else
                        ...
                        c1.log("code ; 444125 wrong file format : " & Pat)
                        c1.renameFile(Pat, 2, Pat)
                        IgnoredFile = IgnoredFile + 1
                    End If
                Else
                    AlreadyUploaded = AlreadyUploaded + 1
                    c1.log("code ; 444633 uploaded before File : " & Pat)
                End If
        ...
        Catch ex As Exception
            c1.log("error code ; 777123  " & ex.Message)
        End Try
    End Sub
    ...
    Private Sub bw1_DoWork()
        files = My.Computer.FileSystem.GetFiles(Label2.Text, FileIO.SearchOption.SearchAllSubDirectories).Where(Function(saa) Path.GetExtension(saa) <> ".Oryx-Dup").ToArray
        Label4.Invoke(Sub()
